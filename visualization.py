...
def IncreaseByTenfold(data):
    ...
    for index, value in enumerate(data):
        expanded_data = np.append(expanded_data, value)
        for i in range(9):
            expanded_data = np.append(expanded_data, value)
...
# Draw threshold
plt.hlines(threshold, colors='black')
...
plt.xticks(np.arange(0, gt_by_all_video_flatten.shape[0], step=50), rotation=45)
for i, value in enumerate(gt_by_all_video_flatten):
    plt.plot(threshold, color='black')  # Draw threshold
    if value == 1:
        plt.axvspan(i, i, alpha=0.7, color='red')
# Draw threshold
plt.hlines(threshold_reconstruct1d, colors='black')
